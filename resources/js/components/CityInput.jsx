...
import { validateVille } from '../utils/validation';
...
// Extrait un nom de ville propre depuis une réponse Nominatim
function extraireVille(address) {
    if (!address) return '';
    return address.city || address.town || address.village ||
           address.municipality || address.county || '';
}

export default function CityInput({ value, onChange, placeholder, error }) {
    ...
    const searchCity = async (searchQuery) => {
        ...
        try {
            ...
            // On ne garde que des noms de villes propres, sans doublons
            const villes = [];
            data.forEach(item => {
                const v = extraireVille(item.address);
                if (v && !villes.some(x => x.nom === v)) {
                    villes.push({ nom: v, display: item.display_name });
                }
            });
    ...
    const handleSelectSuggestion = (ville) => {
        setQuery(ville.nom);
        onChange(ville.nom);
        setShowSuggestions(false);
        setSuggestions([]);
    ...
    // Géolocalisation au clic sur le repère : récupère la ville
        ...
        navigator.geolocation.getCurrentPosition(
            async (pos) => {
                ...
                try {
                    ...
                    const ville = extraireVille(data.address);
                    if (ville) {
                        setQuery(ville);
                        onChange(ville);
                        setShowSuggestions(false);
                        ...
                        setGeoError('Ville introuvable à votre position');
                    }
                } catch (err) {
                    console.error(err);
    ...
    const isValid = query && validateVille(query);
        ...
        <div ref={wrapperRef} style={{ position: 'relative' }}>
            <div style={{ position: 'relative' }}>
                <input
                    ...
                    placeholder={placeholder || 'Ex: Rabat, Maroc'}
                    style={{
                        width: '100%',
            ...
            {showSuggestions && suggestions.length > 0 && (
                <div style={{
                    ...
                    {suggestions.map((ville, index) => (
                        <div
                            key={index}
                            onClick={() => handleSelectSuggestion(ville)}
                            style={{
                                padding: '12px 16px',
                                cursor: 'pointer',
